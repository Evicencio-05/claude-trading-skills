...
def tmp_repo(tmp_path: Path):
    ...
    return {"root": tmp_path, "reports": reports, "logs": logs, "watchlist": watchlist}


def test_find_latest_same_day_picks_latest_timestamp(tmp_repo):
    reports = tmp_repo["reports"]
    as_of = date(2026, 5, 31)
    _write_screener(reports / "vcp_screener_2026-05-31_080928.json", ["A"], 1)
    _write_screener(reports / "vcp_screener_2026-05-31_081301.json", ["B"], 1)
    result = find_latest_same_day(reports, "vcp_screener_", as_of)
...
def test_find_latest_same_day_ignores_prior_day(tmp_repo):
    reports = tmp_repo["reports"]
    as_of = date(2026, 5, 31)
    _write_screener(reports / "vcp_screener_2026-05-30_120000.json", ["A"], 1)
    assert find_latest_same_day(reports, "vcp_screener_", as_of) is None


def test_screener_covers_ticker_by_symbol(tmp_repo):
    path = tmp_repo["reports"] / "vcp.json"
...
def test_find_screener_for_ticker_prefers_covering_file(tmp_repo):
    reports = tmp_repo["reports"]
    as_of = date(2026, 5, 31)
    _write_screener(reports / "vcp_screener_2026-05-31_080928.json", ["MRAM"], 1)
    _write_screener(reports / "vcp_screener_2026-05-31_081301.json", [], 2)
    found = find_screener_for_ticker(reports, "vcp_screener_", "VECO", as_of, ["MRAM", "VECO"])
...
def test_market_context_path(tmp_repo):
    logs = tmp_repo["logs"]
    as_of = date(2026, 5, 31)
    assert market_context_path(logs, as_of) is None
    ctx = logs / "market_context_2026-05-31.md"
    ctx.write_text("# context\n")
    assert market_context_path(logs, as_of) == ctx
...
def test_preflight_manifest_reuses_market_context(tmp_repo):
    logs = tmp_repo["logs"]
    as_of = date(2026, 5, 31)
    (logs / "market_context_2026-05-31.md").write_text("# Pre-Market\n")
...
def test_preflight_manifest_reuses_vcp_when_covered(tmp_repo):
    ...
    logs = tmp_repo["logs"]
    as_of = date(2026, 5, 31)
    (logs / "market_context_2026-05-31.md").write_text("# ctx\n")
    _write_screener(reports / "vcp_screener_2026-05-31_081301.json", [], 2)
    manifest = build_preflight_manifest("VECO", as_of, repo_root=tmp_repo["root"])
