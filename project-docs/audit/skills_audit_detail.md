<!-- DETAIL FILE — Load only when investigating a specific skill.
     Do not load routinely. Operational summary: skills_audit.md -->
...
- Notes: Pure aggregator — no direct API calls. Takes JSON outputs from upstream
  ...
  P2 FIX (2026-05-10): Schema mismatch resolved. Breadth, uptrend, and sector inputs
  now parse correctly. Confidence level with free-tier inputs is LOW (accurate — regime
  and top_risk are in CRITICAL_INPUTS and both require FMP paid tier). Run as-is.
  SCALE DEPENDENCY: At $500 (Portfolio A), the difference between 50% and 80% exposure
  is $150 in dollar risk. At $10K (Portfolio B Roth IRA), 50% vs 80% = $3K difference
  — meaningful. Exposure ceiling now matters for portfolio B. Upgrade run cadence from
  weekly to twice-weekly now that the schema is fixed.
  At $50K+ it is a 5/5. Upgrade this rating explicitly as portfolio grows.
  CURRENT USE: Run twice-weekly with three free CSV inputs. LOW confidence is accurate
  at free tier. Upgrade to MEDIUM when FMP Starter unlocks regime + top_risk inputs.
- Time: 12 min
...
- Notes: No API, standard library only — fully offline. Three modes: Fixed
  ...
  DUAL-PORTFOLIO NOTE (2026-05-10): Two accounts now in scope.
  Portfolio A ($500 taxable): small absolute dollar risk per trade ($5-$10 at 1-2%).
  Portfolio B ($10K Roth IRA): meaningful sizing — $100/trade at 1%. For options in
  Portfolio B, sizing by premium paid (not underlying price) matters — use stop at 50%
  of premium as the risk figure. Account routing: always specify which account when
  sizing so the output references the correct capital base.
...
- Notes: FMP optional — Black-Scholes works fully offline. Covers 17 strategies,
  ...
  DUAL-PORTFOLIO NOTE (2026-05-10): Portfolio B is a Roth IRA (~$10K). At this scale,
  options-strategy-advisor is significantly more relevant — covered calls, cash-secured
  puts, long calls/puts are all IRA-eligible and meaningful at $10K vs $500.
  IRA CONSTRAINT RULE: For every recommendation produced for Portfolio B, the skill
  must explicitly flag: IRA-ELIGIBLE (long calls, long puts, covered calls, cash-secured
  puts) or NOT EXECUTABLE IN IRA (naked selling, spreads requiring margin, undefined risk).
  Non-eligible suggestions should be marked "educational only."
  Upgrade H rating for stock/options from 3 → 4 at Portfolio B scale.
