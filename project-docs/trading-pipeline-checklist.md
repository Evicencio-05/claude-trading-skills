...
| Pre-market skill JSON/MD | `reports/pre_market/` |
...
| Broker snapshot | `reports/portfolio_review_YYYY-MM-DD.md` |
...
- [ ] Read `reports/logs/market_context_YYYY-MM-DD.md` — posture, ceiling, position flags
...
- [ ] **Broker snapshot** — skill `robinhood-portfolio-review` → `reports/portfolio_review_*.md`
...
- **FMP blocks** — `economic-calendar-fetcher` v3 → `scripts/fred_calendar.py`; vcp/canslim watchlist via `--universe` on Starter; full S&P 500 → FMP Professional ($79/mo)

---

## Changelog

- **2026-05-31** — FMP Starter active; stable API fixes; watchlist screener workflows unblocked
